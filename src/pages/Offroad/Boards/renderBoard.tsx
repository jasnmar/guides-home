...
import { bullets, component, layout, layouts } from "./boardInterfaces"
...
function buildComponents(layout : layout, classList: string) : ReactElement {
  if(layout.components) {
    ...
    if(componentList) {
      const renderedComponents : ReactElement[] = componentList.map((component) => {
...
function buildSingleComponent(component : component) {
  ...
  if(component.bullets) {
    fixedBullets = processBullets(component.bullets)
  }
  ...
  switch (component.displayType) {
    case "small-lead":
      renderedComponent = 
        <div key={uuidv4()}>
          <SmallLeadInfo large={component.large} small={component.small} bullets={fixedBullets}>
          </SmallLeadInfo>
        </div>
      break
    case "aside":
      renderedComponent = 
        <div key={uuidv4()}>
          <Aside asideText={component.large} asideBullets={fixedBullets}> 
          </Aside>
        </div>
      break
  }
